...
class OrderController extends Controller
{
    ...
    public function store(OrderRequest $request)
    {
        try {
            ...
            $cart = $this->cart->with('cart_details')->where('uuid', $request->cart_uuid)->get();
            $items = 0;
            $total = 0;
            foreach ($cart as $value) {
                ...
                $orderDetail->price = $value->cart_details->price;
                $orderDetail->save();

                $items += $value->quantity;
                $total += $value->quantity * $value->cart_details->price;
            }
